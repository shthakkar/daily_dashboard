...
def _make_df():
    ...
    # the wrong column would be caught. All three clear the $500M dollar
    # volume (Price * Volume) threshold.
    return pd.DataFrame({
...
def test_rows_from_df_filters_below_min_dollar_volume():
    df = pd.DataFrame({
        "Ticker":     ["AAAA", "BBBB"],
        "Price":      [10.0, 1.0],
        "Change %":   ["5.00%", "5.00%"],
...
def test_rows_from_df_dollar_volume_exactly_at_threshold_is_excluded():
    df = pd.DataFrame({
        "Ticker":     ["AAAA"],
        "Price":      [50.0],
        "Change %":   ["5.00%"],
        "Volume":     [10_000_000.0],  # exactly $500M
        "Avg Volume": [1_000_000.0],
        "Rel Volume": [3.5],
    })
    assert _rows_from_df(df) == []


def test_rows_from_df_excludes_when_price_or_volume_missing():
    # A NaN Price or Volume makes the dollar-volume comparison False, so the
    # row is dropped rather than kept with a fabricated N/A dollar volume.
    df = pd.DataFrame({
        "Ticker":     ["TT0"],
        "Price":      [float("nan")],
        "Change %":   ["5.00%"],
